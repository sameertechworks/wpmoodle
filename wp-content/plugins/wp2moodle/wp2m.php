...
add_action('wp_login', 'wp2moodle_handler');
add_action('wp_logout', 'wp2moodle_logout');
//function on user profile update
add_action('profile_update', 'appusers_init');
...
function my_profile_update($user_id, $old_user_data) {
    ...
    if (isset($_POST) && isset($_POST['user_id'])) {
        ...
        $url = home_url() . "/moodle/auth/wp2moodle/updateUser.php?userPass=$userPass&firstName=$firstName&lastName=$lastName&userEmail=$email&userName=$userName&action=update";
...
function appusers_init($user_id) {
    ...
    require_once (get_home_path() . "/moodle/auth/wp2moodle/updateUser.php");

    if (isset($_POST)) {
        $user = array();
        $user['result'] = $wpdb->get_results('Select * From wp_users Where ID = "' . $user_id . '"');
...
function wp2m_create_menu() {
    add_menu_page(
            __('wp2Moodle', EMU2_I18N_DOMAIN), __('wp2Moodle', EMU2_I18N_DOMAIN), 'administrator', WP2M_PLUGIN_DIRECTORY . '/wp2m_settings_page.php', '', plugins_url('wp2moodle/icon.png', WP2M_PLUGIN_DIRECTORY) //__FILE__));
    );
}
 ...
 */
function wp2moodle_handler($content = null) {
//    do_shortcode('[wp2moodle group="group2"]A hyperlink[/wp2moodle]');

    $atts = array('group' => 'group2');
...
// url = moodle_url + "?data=" + <encrypted-value>
    ...
    if (is_user_logged_in()) {
        echo "<iframe id='login' src='" . wp2moodle_generate_hyperlink($cohort, $group) . "' style='display:none'></iframe>";
    }
    return $url;
}
...
function wp2m_add_button() {
    do_shortcode('[wp2moodle group="group2"][/wp2moodle]');
